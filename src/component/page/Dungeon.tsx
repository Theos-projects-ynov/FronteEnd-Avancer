...
import type { Trainer, OwnedPokemon } from "../../type/Trainer";
...
const dungeons: Dungeon[] = [
  ...
  },
  {
    ...
    name: "Caverne sombre",
    difficulty: 3,
    type: "ténèbres et roche",
    rewards: ["Super Ball", "Rappel", "XP"]
  },
  {
    id: 3,
    name: "Pic gelé",
...
function DungeonPage() {
  ...
  useEffect(() => {
    if (trainer?.pokemons) {
      setAvailablePokemon(trainer.pokemons.filter(
        pokemon => !selectedPokemon.find(selected => selected.id === pokemon.id)
      ));
    }
            ...
            <CardContent>
              ...
              <div className="selected-pokemon-grid">
                {Array.from({ length: 4 }, (_, index) => {
                  ...
                  return (
                    <Card 
                      key={index} 
                      className={`selected-slot ${pokemon ? 'filled' : 'empty'}`}
                      onClick={() => pokemon && handlePokemonDeselect(pokemon)}
                    >
                      <CardContent>
                        {pokemon ? (
                          <div className="pokemon-content">
                            <Typography variant="subtitle2" className="pokemon-name">
              ...
              <Box className="start-button-container">
                <Button
                  ...
                  className={`start-dungeon-button ${isTeamReady ? 'ready' : 'disabled'}`}
                >
                  {isTeamReady ? "Commencer le donjon" : `Sélectionnez ${4 - selectedPokemon.length} Pokémon de plus`}
                </Button>
              </Box>
            </CardContent>
          </Card>
